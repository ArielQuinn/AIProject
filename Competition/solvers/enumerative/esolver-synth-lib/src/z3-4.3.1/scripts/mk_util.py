...
def find_all_deps_rec(name, deps, accum_deps):
    for dep in deps:
        if dep in _ComponentNames:
            dep_comp = get_component(dep)
            if dep_comp not in accum_deps:
                accum_deps.add(dep)
                find_all_deps_rec(dep_comp.name, dep_comp.deps, accum_deps)
        else:
            raise MKException("Unknown component '%s' at '%s'." % (dep, name))
    return [ dep for dep in accum_deps ]
...
# audupa: changed to recursive implementation
def find_all_deps(name, deps):
    return find_all_deps_rec(name, deps, set())
...
class Component:
    ...
    def add_cpp_rules(self, out, include_defs, cppfile):
        ...
        out.write('\n')
        if SHOW_CPPS:
            out.write('\t@echo "%s/%s --> %s/%s$(OBJ_EXT)"\n' % (self.src_dir, cppfile, self.build_dir, os.path.splitext(cppfile)[0]))
        ...
        if TRACE and not DEBUG_MODE:
            if IS_WINDOWS:
                ...
                extra_opt = '-D _TRACE'
        out.write('ifeq "x$(VERBOSE_MAKE)" "x"\n')
        ...
        # audupa: LTO build
        ltoobjfile = '%s/%s$(LTO_OBJ_EXT)' % (self.build_dir, os.path.splitext(cppfile)[0])
        srcfile = '%s/%s' % (self.to_src_dir, cppfile)
        out.write('%s: ' % ltoobjfile)
        self.add_cpp_h_deps(out, cppfile)
        out.write('\n')
        if SHOW_CPPS:
            out.write('\t@echo "%s/%s --> %s/%s$(LTO_OBJ_EXT)"\n' % (self.src_dir, cppfile, self.build_dir, os.path.splitext(cppfile)[0]))
        # TRACE is enabled in debug mode by default
        extra_opt = ''
        if TRACE and not DEBUG_MODE:
            if IS_WINDOWS:
                ...
                extra_opt = '-D _TRACE'
        out.write('ifeq "x$(VERBOSE_MAKE)" "x"\n')
        out.write('\t\t@$(CXX) $(CXXFLAGS) $(LTOFLAG) %s $(%s) $(CXX_OUT_FLAG)%s %s\n' % (extra_opt, include_defs, ltoobjfile, srcfile))
        out.write('else\n')
        out.write('\t$(CXX) $(CXXFLAGS) $(LTOFLAG) %s $(%s) $(CXX_OUT_FLAG)%s %s\n' % (extra_opt, include_defs, ltoobjfile, srcfile))
        out.write('endif\n')

    def get_objs_internal(self, objset, depset):
        for cppfile in get_cpp_files(self.src_dir):
            objset.add('%s/%s$(OBJ_EXT)' % (self.build_dir, os.path.splitext(cppfile)[0]))
        for dep in self.deps:
            if dep not in depset:
                depset.add(dep)
                get_component(dep).get_objs_internal(objset, depset)
        return

    def get_ltoobjs_internal(self, objset, depset):
        for cppfile in get_cpp_files(self.src_dir):
            objset.add('%s/%s$(LTO_OBJ_EXT)' % (self.build_dir, os.path.splitext(cppfile)[0]))
        for dep in self.deps:
            if dep not in depset:
                depset.add(dep)
                get_component(dep).get_ltoobjs_internal(objset, depset)
        return
        
    def get_objs(self):
        objs = set()
        self.get_objs_internal(objs, set())
        return objs

    def get_ltoobjs(self):
        objs = set()
        self.get_ltoobjs_internal(objs, set())
        return objs
...
class LibComponent(Component):
    ...
    def mk_makefile(self, out):
        ...
        for cppfile in get_cpp_files(self.src_dir):
            ...
            ltoobjfile = '%s/%s$(LTO_OBJ_EXT)' % (self.build_dir, os.path.splitext(cppfile)[0])
            objs.append(objfile)
            ltoobjs.append(ltoobjfile)

        libfile = '%s/%s$(LIB_EXT)' % (self.build_dir, self.name)
        ltolibfile = '%s/%s$(LTO_LIB_EXT)' % (self.build_dir, self.name)
        out.write('%s:' % libfile)
        for obj in objs:
            out.write(' ')
            out.write(obj)
        ...
        out.write('ifeq "x$(VERBOSE_MAKE)" "x"\n')
        out.write('\t@echo "%s"\n' % ltolibfile)
        out.write('\t@$(AR) $(AR_FLAGS) $(AR_OUTFLAG)%s' % ltolibfile)
        for obj in ltoobjs:
            out.write(' ')
            out.write(obj)
        out.write('\n')
...
class ExeComponent(Component):
    ...
    def mk_makefile(self, out):
        ...
        # audupa: changed to use lto
        exefile = '%s$(EXE_EXT)' % self.exe_name
        out.write('%s:' % exefile)
        deps = sort_components(self.deps)
        objs = []
        for cppfile in get_cpp_files(self.src_dir):
            objfile = '%s/%s$(LTO_OBJ_EXT)' % (self.build_dir, os.path.splitext(cppfile)[0])
            objs.append(objfile)
        for obj in objs:
            out.write(' ')
            out.write(obj)
        for dep in deps:
            c_dep = get_component(dep)
            out.write(' %s/%s$(LTO_LIB_EXT)' % (c_dep.build_dir, c_dep.name))
        out.write('\n')
    ...
    def mk_makefile(self, out):
        ...
        for reexport in self.reexports:
            ...
            for cppfile in get_cpp_files(reexport.src_dir):
                objfile = '%s/%s$(LTO_OBJ_EXT)' % (reexport.build_dir, os.path.splitext(cppfile)[0])
                objs.append(objfile)
        for obj in objs:
            out.write(' ')
            out.write(obj)
...
class StaticLibComponent(Component):
    def __init__ (self, name, staticlib_name, path, deps, install):
        Component.__init__(self, name, path, deps)
        if staticlib_name == None:
            staticlib_name = name
        self.staticlib_name = staticlib_name
        self.install = install

    def mk_makefile(self, out):
        Component.mk_makefile(self, out)
        # generate rule for $(LIB_EXT)
        libfile = '%s$(LIB_EXT)' % self.staticlib_name
        out.write('%s:' % libfile)
        ...
        # lto lib
        libfile = '%s$(LTO_LIB_EXT)' % self.staticlib_name
        out.write('%s:' % libfile)
        for obj in self.get_ltoobjs():
            out.write(' %s' % obj)
        out.write('\n')
        out.write('\t$(AR) $(AR_FLAGS) $(AR_OUTFLAG)%s' % libfile)
        for obj in self.get_ltoobjs():
    ...
    # All StaticLibs are included in the all: rule
    def main_component(self):
        return True
...
# audupa: static libs
def add_static_lib(name, deps=[], path=None, staticlib_name = None, install=True):
    c = StaticLibComponent(name, staticlib_name, path, deps, install)
    reg_component(name, c)
